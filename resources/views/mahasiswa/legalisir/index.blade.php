    ...
    <div class="">
                ...
                <div class="row-span-3 border border-gray-200 rounded-2xl p-5 bg-white">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <p class="text-gray-700 font-semibold">{{ Auth::user()->nim }}</p>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <p class="text-gray-700 font-semibold">{{ Auth::user()->student->tempat_lahir ?? '-' }},
                                {{ Auth::user()->student->tanggal_lahir ?? '-' }}</p>
                        </div>
                        <div>
                            <p class="text-gray-500 text-sm">Program Studi</p>
                            <p class="text-gray-700 font-semibold">{{ Auth::user()->student->program_studi ?? '-' }}</p>
                        </div>
                        <div>
                            <p class="text-gray-500 text-sm">No SK Rektor</p>
                            <p class="text-gray-700 font-semibold">{{ Auth::user()->student->nomor_sk_rektor ?? '-' }}</p>
                        </div>
                        <div>
                            <p class="text-gray-500 text-sm">No Ijazah</p>
                            <p class="text-gray-700 font-semibold">{{ Auth::user()->student->nomor_ijazah ?? '-' }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="mt-3 border border-gray-200 rounded-2xl p-5 bg-white">
            <div class="flex justify-between items-center">
                ...
                @if (isset($file_ijazah) && isset($file_transkrip_1))
                    <a href="{{ route('mahasiswa.transaksi.create') }}"
                        class="bg-cyan-600 text-white px-6 py-2 h-fit rounded-xl hover:bg-cyan-700 hover:shadow transition-all duration-300">Ajukan
                        Legalisir</a>
                @endif
            </div>
            <hr class="my-3">
                ...
                <div class="mb-6">
                    ...
                    <div class="border p-2 rounded-xl bg-gray-50 flex gap-3 items-center">
                        ...
                        <button type="button" onclick="document.getElementById('file_ijazah').click()"
                            class="border p-2 rounded-xl bg-cyan-500/10 border-cyan-500/50 text-cyan-600 w-1/2">
                            Pilih File Ijazah
                        </button>
                    </div>
                </div>
