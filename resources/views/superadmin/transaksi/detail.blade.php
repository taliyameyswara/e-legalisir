...
@section('content')
    <div class="p-5 bg-white rounded-2xl border border-gray-200">
        <a href="{{ route('superadmin.transaksi.index') }}" class="text-cyan-600 hover:underline">
            ← Kembali </a>
            ...
            <div class="">
                ...
                <div class="p-3 mt-4 w-full bg-gray-50 rounded-xl border">
                    ...
                    @if ($transaction->status == 'proses legalisir')
                            <form action="{{ route('superadmin.transaksi.approve', $transaction->id) }}" method="POST">
                                @csrf
                    ...
                    <div class="flex gap-x-2 items-center">
                        <form action="{{ route('superadmin.transaksi.acc', $transaction->id) }}" method="POST">
                            @csrf
                            <button type="submit"
                                class="px-4 py-2 mt-1 text-sm text-white bg-cyan-600 rounded-lg hover:bg-cyan-700 w-fit h-fit">
                                Terima Dokumen
                        ...
                        <div class="p-6 w-full max-w-md bg-white rounded-xl shadow-lg">
                            ...
                            <form action="{{ route('superadmin.transaksi.tolak', $transaction->id) }}" method="POST">
                                @csrf
